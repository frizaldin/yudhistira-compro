...
class OpenTicketChatApiController extends Controller
{
    /**
     * GET /api/v1/open-tickets/chats?ticket_id=1
     * Daftar chat tiket (hanya tiket milik guru yang login).
     */
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $ticket = OpenTicket::where('user_id', $teacher->id)->find($ticketId);
            if (!$ticket) {
                return response()->json([
                    'code' => 404,
                    'success' => false,
            ...
            $chats = OpenTicketChat::where('open_ticket_id', $ticketId)
                ->with('attachments')
    ...
    /**
     * POST /api/v1/open-tickets/chats/store
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            $request->validate([
                'ticket_id' => 'required|integer|exists:open_tickets,id',
    ...
    /**
     * PUT /api/v1/open-tickets/chats/update/{chatId}
    ...
    public function update(Request $request, int $chatId): JsonResponse
    {
        try {
            ...
            $chat = OpenTicketChat::where('user_id', $teacher->id)
                ->where('viewpoint', 'user')
                ->with('openTicket', 'attachments')
                ->find($chatId);
    ...
    /**
     * DELETE /api/v1/open-tickets/chats/delete/{chatId}
