                ...
                <div class="card">
                    <div class="card-body">
                        <form method="get" class="mb-3">
                            <input type="text" name="search" class="form-control d-inline-block w-auto" placeholder="Cari title/judul..." value="{{ request('search') }}">
                                ...
                                <tbody>
                                    @foreach ($collection as $item)
                                        <tr id="data-{{ $item->id }}">
                                            ...
                                            <td>
                                                @if($item->thumbnail)
                                                    <img style="width: 60px;" src="{{ asset($item->thumbnail) }}" alt="">
                                                @else
                                                    -
                                                @endif
                                            </td>
                                            <td>{{ $item->title }}</td>
                                            <td>{{ $item->judul ?? '-' }}</td>
                                            <td>{{ $item->file ? basename($item->file) : '-' }}</td>
