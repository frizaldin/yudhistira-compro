<x-backend.layouts>
    <div class="container-fluid">
        <div class="page-title-box d-md-flex justify-content-md-between align-items-center">
            <h4 class="page-title">{{ $title }}@if ($subcategory)
                    - {{ $subcategory->title }}
                @endif
            </h4>
            <div class="d-flex align-items-center">
                <div class="me-3">
                    <a href="{{ $subcategory_id ? $base_url . '/' . $subcategory_id . '/add' : $base_url . '/add' }}"
                ...
                <div class="">
                    <ol class="breadcrumb mb-0">
                        ...
                        @if ($subcategory)
                            <li class="breadcrumb-item"><a href="{{ url('backend/digital-product') }}">Digital
                                    Product</a></li>
                        @endif
                                ...
                                <tbody>
                                    @foreach ($collection as $item)
                                        <tr id="data-{{ $item->id }}">
                                            ...
                                            @if (!$subcategory_id)
                                                <td>
                                                    {{ $item->digitalProduct ? $item->digitalProduct->title : '-' }}
                                                </td>
                                            @endif
                                            <td>
                                                {{ $item->order }}
