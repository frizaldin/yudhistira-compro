                ...
                <div class="card-body">
                    <form id="basic-form" action="{{ url('backend/configuration/update') }}" class="_form"
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <div class="d-flex" style="gap:10px">
                                                    @php
                                                        $start_request = date('Y-m-d', strtotime(date('Y').'-'.$item->start_request));
                                                    @endphp
                                                    <!-- Bulan -->
                                                    <select name="start_month" class="form-control">
                                                        ...
                                                        @foreach(range(1,12) as $m)
                                                            <option value="{{ $m }}"
                                                                {{ isset($item) && date('m', strtotime($start_request)) == $m ? 'selected' : '' }}>
                                                    ...
                                                    <select name="start_day" class="form-control">
                                                        ...
                                                        @foreach(range(1,31) as $d)
                                                            <option value="{{ $d }}"
                                                                {{ isset($item) && date('d', strtotime($start_request)) == $d ? 'selected' : '' }}>
                                                                {{ $d }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <div class="d-flex" style="gap:10px">
                                                    @php
                                                        $end_request = date('Y-m-d', strtotime(date('Y').'-'.$item->end_request . ' +1 year'));
                                                    @endphp
                                                    <select name="end_month" class="form-control">
                                                        <option value="">Bulan</option>
                                                        @foreach(range(1,12) as $m)
                                                            <option value="{{ $m }}"
                                                                {{ isset($item) && date('m', strtotime($end_request)) == $m ? 'selected' : '' }}>
                                                                {{ date('F', mktime(0,0,0,$m,1)) }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                    ...
                                                    <select name="end_day" class="form-control">
                                                        ...
                                                        @foreach(range(1,31) as $d)
                                                            <option value="{{ $d }}"
                                                                {{ isset($item) && date('d', strtotime($end_request)) == $d ? 'selected' : '' }}>
                                                                {{ $d }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="number" name="max_request" class="form-control"
                                                    value="{{ $item->max_request ?? '' }}">
                                            </div>
                                        </div>
                        ...
                        @else
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <textarea name="certificate_temp" class="form-control" rows="6"
                                                    placeholder="Masukkan template sertifikat...">{{ $item->certificate_temp ?? '' }}</textarea>
                                                <small class="text-muted">Template yang digunakan untuk generate sertifikat.</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <h3>Konfigurasi Halaman Selesai Event</h3>
                                </div>
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label>Base URL Frontend Halaman Selesai</label>
                                                <input type="text" name="event_completion_base_url" class="form-control"
                                                    value="{{ $item->event_completion_base_url ?? '' }}" placeholder="https://teacher-hub.com/redeem">
                                                <small class="text-muted">Contoh: https://teacher-hub.com/redeem. Ini akan digunakan untuk membuat link saat meng-copy token di daftar event.</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
