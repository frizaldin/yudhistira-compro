    ...
    <div class="row clearfix">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form id="basic-form" action="{{ url('backend/user/update') }}" class="_form" method="post">
                        @csrf
                        <input type="hidden" name="id" value="{{ $item->id }}">
                        <div class="form-group">
                            <label>Otoritas</label>
                            <select class="form-select form-control" name="authorities_id">
                                @foreach ($authority as $row)
                                    <option value="{{ $row->id }}"
                                        {{ $item->authorities_id == $row->id ? 'selected' : '' }}>{{ $row->title }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
